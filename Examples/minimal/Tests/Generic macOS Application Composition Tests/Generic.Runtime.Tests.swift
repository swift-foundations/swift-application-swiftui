...
extension Application.Runtime.Scene {
    @Suite
    struct Test {
        /// The table is asserted against the whole example, not against the
        /// default's own definition: a conformer that overrode it would still be
        /// reported here, which is the property worth holding.
        @Test
        ...
        func bootComposesARootFromASuppliedEnvironment() async {
            let root = await Generic.Runtime.boot(EnvironmentSnapshot([:]))
